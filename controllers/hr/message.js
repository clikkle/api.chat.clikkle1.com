...
console.log(adminId,'adminIdadminIdadminId')
    ...
    const query = {
      // adminId: adminId,
      $or: [
        { sender: new Types.ObjectId(adminId), receiver: new Types.ObjectId(receiver) },
        { sender: new Types.ObjectId(receiver), receiver: new Types.ObjectId(adminId) }
      ]
    };
        ...
        const pipeline = [
          {
            $match: {
              ...
              $or: [
                ...
                { receiver: new Types.ObjectId(adminId) }
              ]
            }
          },
          {
...
messageRouter.delete('/delete', async (req, res, next) => {
  try {
    const { selectedchats } = req.body;
    const senderId = new Types.ObjectId(req.orgId);

    const result = await Message.deleteMany({
      _id: { $in: selectedchats },
      $or: [
        { sender: senderId },
        { receiver: senderId }
      ]
...
messageRouter.delete('/deleteuserchat/:uid', async (req, res, next) => {
  try {
    const { uid } = req.params;
    const senderId = new Types.ObjectId(req.orgId);
    const result = await Message.deleteMany({
      $or: [
        { sender: senderId, receiver: new Types.ObjectId(uid) },
        { sender: new Types.ObjectId(uid), receiver: senderId }
      ]
    });
...
messageRouter.get('/unread', async (req, res, next) => {
  try {
    ...
    const totalUnread = await Message.countDocuments({
      // _id: { $in: selectedchats.map((id) => new ObjectId(id)) },
